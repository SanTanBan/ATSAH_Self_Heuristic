...
def PuLP_Algorithm(max_seconds_allowed_for_calculation=0,directory_containing_Node_Specifications_file="",directory__containing_Vehicle_Specifications_file="",directory_containing_Distance_Matrices_file="",directory_details_for_saving=""):
    ...
    if max_seconds_allowed_for_calculation>0:
        textfile.write("\t The Status of the problem is "+p.LpStatus[prob.status]+" which was stopped before the given time-limit of "+str(max_seconds_allowed_for_calculation)+".\n")
        textfile.write("\t The Objective Value is "+str(objec_val)+" obtained within "+str(pulp_end_time-pulp_start_time)+" seconds possibly with some Relative Gap... \n \n \n")
    else:
